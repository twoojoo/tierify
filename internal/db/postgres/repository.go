...
func NewRepositories(connectionString string, maxOpenConns, maxIdleConns int, connMaxLifetime time.Duration) (*repositories.Repositories, error) {
...
func (r *tierRepo) Create(ctx context.Context, tier *models.Tier, limits []models.LimitDefinition) error {
	...
	_, err = tx.ExecContext(ctx,
		`INSERT INTO tiers (id, key, name, version, deprecated, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7)`,
		tier.ID, tier.Key, tier.Name, tier.Version, tier.Deprecated, tier.CreatedAt, tier.UpdatedAt,
	)
	if err != nil {
		return err
	}
	...
	for _, l := range limits {
		...
		_, err = tx.ExecContext(ctx,
			`INSERT INTO limit_definitions (id, tier_id, key, kind, scope, config, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7)`,
...
func (r *tierRepo) GetByKey(ctx context.Context, key string) (*models.Tier, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, key, name, version, deprecated, created_at, updated_at FROM tiers WHERE key = $1 AND deprecated = FALSE ORDER BY version DESC LIMIT 1`, key)
	return scanTier(row)
}

func (r *tierRepo) GetByKeyAndVersion(ctx context.Context, key string, version int) (*models.Tier, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, key, name, version, deprecated, created_at, updated_at FROM tiers WHERE key = $1 AND version = $2`, key, version)
	return scanTier(row)
}
...
func (r *tierRepo) ListVersions(ctx context.Context, key string, offset, limit int) (*repositories.PaginatedResult[models.Tier], error) {
	var total int
	r.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM tiers WHERE key = $1`, key).Scan(&total)

	rows, err := r.db.QueryContext(ctx,
		`SELECT id, key, name, version, deprecated, created_at, updated_at FROM tiers WHERE key = $1 ORDER BY version DESC LIMIT $2 OFFSET $3`,
		key, limit, offset)
	if err != nil {
		return nil, err
	}
...
func (r *tierRepo) List(ctx context.Context, filters models.TierFilters) (*repositories.PaginatedResult[models.Tier], error) {
	...
	query := fmt.Sprintf(`SELECT id, key, name, version, deprecated, created_at, updated_at FROM tiers %s ORDER BY key, version DESC LIMIT $%d OFFSET $%d`,
...
func (r *tierRepo) Deprecate(ctx context.Context, key string, version int) error {
	_, err := r.db.ExecContext(ctx,
		`UPDATE tiers SET deprecated = TRUE, updated_at = $1 WHERE key = $2 AND version = $3`,
...
func (r *limitDefRepo) GetByTier(ctx context.Context, tierID string) ([]models.LimitDefinition, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, tier_id, key, kind, scope, config, created_at FROM limit_definitions WHERE tier_id = $1`, tierID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *limitDefRepo) GetByTierAndKey(ctx context.Context, tierID, limitKey string) (*models.LimitDefinition, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, tier_id, key, kind, scope, config, created_at FROM limit_definitions WHERE tier_id = $1 AND key = $2`, tierID, limitKey)
...
func (r *tenantRepo) Create(ctx context.Context, tenant *models.Tenant) error {
	...
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO tenants (id, external_id, parent_id, tier_id, tier_version, status, metadata, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
...
func (r *tenantRepo) GetByID(ctx context.Context, id string) (*models.Tenant, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, external_id, parent_id, tier_id, tier_version, status, metadata, created_at, updated_at FROM tenants WHERE id = $1`, id)
	return scanTenant(row)
}

func (r *tenantRepo) GetByExternalID(ctx context.Context, externalID string) (*models.Tenant, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, external_id, parent_id, tier_id, tier_version, status, metadata, created_at, updated_at FROM tenants WHERE external_id = $1`, externalID)
	return scanTenant(row)
}
...
func (r *tenantRepo) GetChildren(ctx context.Context, tenantID string, offset, limit int) (*repositories.PaginatedResult[models.Tenant], error) {
	var total int
	r.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM tenants WHERE parent_id = $1`, tenantID).Scan(&total)

	rows, err := r.db.QueryContext(ctx,
		`SELECT id, external_id, parent_id, tier_id, tier_version, status, metadata, created_at, updated_at FROM tenants WHERE parent_id = $1 LIMIT $2 OFFSET $3`,
...
func (r *tenantRepo) GetAncestors(ctx context.Context, tenantID string) ([]models.Tenant, error) {
	rows, err := r.db.QueryContext(ctx, `
		WITH RECURSIVE ancestors AS (
			SELECT t.* FROM tenants t WHERE t.id = (SELECT parent_id FROM tenants WHERE id = $1)
			UNION ALL
...
func (r *tenantRepo) ListByTierID(ctx context.Context, tierID string) ([]models.Tenant, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, external_id, parent_id, tier_id, tier_version, status, metadata, created_at, updated_at FROM tenants WHERE tier_id = $1`, tierID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *tenantRepo) Update(ctx context.Context, tenant *models.Tenant) error {
	metadataJSON, _ := json.Marshal(tenant.Metadata)
	_, err := r.db.ExecContext(ctx,
		`UPDATE tenants SET external_id = $1, parent_id = $2, tier_id = $3, tier_version = $4, status = $5, metadata = $6, updated_at = $7 WHERE id = $8`,
...
func (r *tenantRepo) Delete(ctx context.Context, tenantID string, mode models.DeleteMode) error {
	...
	if mode == models.DeleteModeOrphan {
		_, err = tx.ExecContext(ctx, `UPDATE tenants SET parent_id = NULL WHERE parent_id = $1`, tenantID)
		if err != nil {
			return err
		}
	} else {
...
func (r *tenantRepo) RecordTierChange(ctx context.Context, change *models.TenantTierChange) error {
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO tenant_tier_history (id, tenant_id, previous_tier_id, previous_version, new_tier_id, new_version, usage_reset, changed_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
		change.ID, change.TenantID, change.PreviousTierID, change.PreviousVersion, change.NewTierID, change.NewVersion, change.UsageReset, change.ChangedAt,
	)
	return err
}
...
func (r *tenantRepo) GetTierHistory(ctx context.Context, tenantID string, offset, limit int) (*repositories.PaginatedResult[models.TenantTierChange], error) {
	var total int
	r.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM tenant_tier_history WHERE tenant_id = $1`, tenantID).Scan(&total)

	rows, err := r.db.QueryContext(ctx,
		`SELECT id, tenant_id, previous_tier_id, previous_version, new_tier_id, new_version, usage_reset, changed_at FROM tenant_tier_history WHERE tenant_id = $1 ORDER BY changed_at DESC LIMIT $2 OFFSET $3`,
		tenantID, limit, offset)
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&h.ID, &h.TenantID, &h.PreviousTierID, &h.PreviousVersion, &h.NewTierID, &h.NewVersion, &h.UsageReset, &h.ChangedAt); err != nil {
			return nil, err
		}
...
func (r *usageRepo) GetUsage(ctx context.Context, tenantID, limitKey string) (*models.UsageRecord, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, tenant_id, limit_key, current_value, window_start, window_end, updated_at FROM usage_records WHERE tenant_id = $1 AND limit_key = $2`,
		tenantID, limitKey)
...
func (r *usageRepo) GetAllUsage(ctx context.Context, tenantID string) ([]models.UsageRecord, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, tenant_id, limit_key, current_value, window_start, window_end, updated_at FROM usage_records WHERE tenant_id = $1`, tenantID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *usageRepo) UpsertUsage(ctx context.Context, record *models.UsageRecord) error {
	_, err := r.db.ExecContext(ctx, `
		...
		DO UPDATE SET current_value = EXCLUDED.current_value, window_start = EXCLUDED.window_start, window_end = EXCLUDED.window_end, updated_at = EXCLUDED.updated_at`,
...
func (r *usageRepo) ResetUsage(ctx context.Context, tenantID, limitKey string) error {
	_, err := r.db.ExecContext(ctx, `DELETE FROM usage_records WHERE tenant_id = $1 AND limit_key = $2`, tenantID, limitKey)
	return err
}

func (r *usageRepo) ResetAllUsage(ctx context.Context, tenantID string) error {
	_, err := r.db.ExecContext(ctx, `DELETE FROM usage_records WHERE tenant_id = $1`, tenantID)
	return err
}
...
func (r *transactionRepo) Create(ctx context.Context, txn *models.Transaction) error {
	...
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO transactions (id, tenant_id, limit_keys, status, ttl_seconds, created_at, expires_at) VALUES ($1, $2, $3, $4, $5, $6, $7)`,
...
func (r *transactionRepo) GetByID(ctx context.Context, id string) (*models.Transaction, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, tenant_id, limit_keys, status, ttl_seconds, created_at, expires_at FROM transactions WHERE id = $1`, id)
	...
	ops, err := r.db.QueryContext(ctx,
		`SELECT limit_key, operation, amount FROM transaction_operations WHERE transaction_id = $1`, id)
	if err != nil {
		return nil, err
	}
	defer ops.Close()
...
func (r *transactionRepo) AddOperation(ctx context.Context, txID string, op models.TransactionOp) error {
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO transaction_operations (transaction_id, limit_key, operation, amount) VALUES ($1, $2, $3, $4)`,
...
func (r *transactionRepo) AddLimitKeys(ctx context.Context, txID string, keys []string) error {
	...
	limitKeysJSON, _ := json.Marshal(txn.LimitKeys)
	_, err = r.db.ExecContext(ctx,
		`UPDATE transactions SET limit_keys = $1 WHERE id = $2`, string(limitKeysJSON), txID)
	return err
}

func (r *transactionRepo) UpdateStatus(ctx context.Context, txID string, status models.TransactionStatus) error {
	_, err := r.db.ExecContext(ctx,
		`UPDATE transactions SET status = $1 WHERE id = $2`, string(status), txID)
	return err
}
...
func (r *transactionRepo) GetPendingByTenantAndLimitKey(ctx context.Context, tenantID, limitKey string) (*models.Transaction, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id FROM transactions WHERE tenant_id = $1 AND status = 'pending'`, tenantID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func scanTier(row *sql.Row) (*models.Tier, error) {
	var t models.Tier
	err := row.Scan(&t.ID, &t.Key, &t.Name, &t.Version, &t.Deprecated, &t.CreatedAt, &t.UpdatedAt)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	if err != nil {
...
func scanTiers(rows *sql.Rows) ([]models.Tier, error) {
	...
	for rows.Next() {
		var t models.Tier
		if err := rows.Scan(&t.ID, &t.Key, &t.Name, &t.Version, &t.Deprecated, &t.CreatedAt, &t.UpdatedAt); err != nil {
			return nil, err
		}
...
func buildTierFilters(filters models.TierFilters) (string, []any) {
	...
	if filters.Key != nil {
		conditions = append(conditions, fmt.Sprintf("key = $%d", paramIdx))
		args = append(args, *filters.Key)
		paramIdx++
	}
	if filters.Deprecated != nil {
		conditions = append(conditions, fmt.Sprintf("deprecated = $%d", paramIdx))
